...
impl ByteCompiler<'_> {
    pub(crate) fn compile_object_literal(&mut self, literal: &ObjectLiteral, dst: &Register) {
        self.bytecode_emitter.emit_push_empty_object(dst.variable());
                ...
                PropertyDefinition::MethodDefinition(m) => {
                    ...
                    match m.name() {
                        PropertyName::Literal(name) => {
                            ...
                            match kind {
                                MethodKind::Get => {
                                    self.bytecode_emitter.emit_set_property_getter_by_name(
                                        dst.variable(),
                                        method.variable(),
                                        index.into(),
                                    )
                                ...
                                MethodKind::Set => {
                                    self.bytecode_emitter.emit_set_property_setter_by_name(
                                        dst.variable(),
                                        method.variable(),
                                        index.into(),
                                    )
    ...
    fn compile_object_literal_computed_method(
        ...
        match kind {
            MethodKind::Get => self.bytecode_emitter.emit_set_property_getter_by_value(
                method.variable(),
                key.variable(),
                object.variable(),
            ),
            MethodKind::Set => self.bytecode_emitter.emit_set_property_setter_by_value(
                method.variable(),
                key.variable(),
                object.variable(),
            ),
            MethodKind::Ordinary => self.bytecode_emitter.emit_define_own_property_by_value(
                method.variable(),
                key.variable(),
                object.variable(),
            ),
